            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    for (int i = 0; i < list.length(); i++) {
                        ...
                        items.setId(weather.getInt("id"));
                        items.setName(weather.getString("name"));
                        items.setCurrentWeather(weather.getJSONArray("weather").getJSONObject(0).getString("main"));
                        items.setDescription(weather.getJSONArray("weather").getJSONObject(0).getString("description"));
                        double templnCelcius = weather.getJSONObject("main").getDouble("temp");
                        items.setTemperature(new DecimalFormat("##.##").format(templnCelcius));
                        listItems.add(items);
                    }
